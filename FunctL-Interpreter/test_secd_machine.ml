...
let test_secd name (expr:Secd_machine.expr) =
  Printf.printf "=== SECD test: %s ===\n" name;
  Printf.printf "Expression: %s\n" (Secd_machine.string_of_expr expr);
  let result = evaluate_secd expr in
  Printf.printf "Result: %s\n\n" (Secd_machine.string_of_value result)
...
let run_tests () =
  (* SECD machine tests *)
  test_secd "Identity" identity;
  test_secd "Identity applied to itself" id_app_id;
  test_secd "True" lc_true;
  test_secd "False" lc_false;
  test_secd "Church numeral 0" church_0;
  test_secd "Church numeral 1" church_1;
  test_secd "Successor of 1" test_succ_1
