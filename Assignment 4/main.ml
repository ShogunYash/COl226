 (* main.ml - Read input file, parse it, type check it, and output AST *)
open Ast
open Lexer
open Parser
open Ast_type_checker  (* Import the type checker module *)
...
(* Parse and type check a string, then print results *)
...
(* Run test cases *)
let run_tests ~with_type_check =
  print_endline "Running test cases...\n";
  
  let print_func = if with_type_check then parse_type_check_and_print else parse_and_print in
...
let () =
  ...
  ] (fun _ -> ()) "Matrix/Vector Language Parser";
  
  if !run_test_cases then run_tests ~with_type_check:!with_type_check;
  ...
  if !process_file then
    try
      ...
      let ast = 
        if !type_check_only || !with_type_check then
          parse_and_check_file input_file
        else
